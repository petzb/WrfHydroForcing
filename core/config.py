...
class ConfigOptions:
    ...
    def read_config(self):
        ...
        try:
            ...
            err_handler.err_out_screen('Missing Input[i][\'Type\'] in Input map in the configuration file.')
        if len(self.input_force_types) != self.number_inputs:
            err_handler.err_out_screen('Number of Input Types must match the number '
                                       'of Input Forcings in the configuration file.')
        ...
        for temporalInterpOpt in self.forceTemoralInterp:
            if temporalInterpOpt < 0 or temporalInterpOpt > 2:
                err_handler.err_out_screen('Invalid Input[i][\'TemporalInterp\'] chosen in the configuration file. '
        ...
        for optTmp in self.t2BiasCorrectOpt:
            if optTmp < 0 or optTmp > 4:
                err_handler.err_out_screen('Invalid Input[i][\'BiasCorrection\'][\'Temperature\'] options specified in the '
                                           'configuration file.')
        ...
        for optTmp in self.psfcBiasCorrectOpt:
            if optTmp < 0 or optTmp > 1:
                err_handler.err_out_screen('Invalid Input[i][\'BiasCorrection\'][\'Pressure\'] options specified in the '
                                           'configuration file.')
            if optTmp == 1:
        ...
        for optTmp in self.windBiasCorrect:
            if optTmp < 0 or optTmp > 4:
                err_handler.err_out_screen('Invalid Input[\'BiasCorrection\'][\'Wind\'] options specified in the configuration file.')
            if optTmp == 1:
                # We are running NWM-Specific bias-correction of CFSv2 that needs to take place prior to regridding.
                self.runCfsNldasBiasCorrect = True
        ...
        for optTmp in self.t2dDownscaleOpt:
            if optTmp < 0 or optTmp > 2:
                err_handler.err_out_screen('Invalid  Input[i][\'Downscaling\'][\'Temperature\'] options specified in '
        ...
        for optTmp in self.psfcDownscaleOpt:
            if optTmp < 0 or optTmp > 1:
                err_handler.err_out_screen('Invalid Input[i][\'Downscaling\'][\'Pressure\'] options specified in the configuration file.')
        ...
        for optTmp in self.swDownscaleOpt:
            if optTmp < 0 or optTmp > 1:
                err_handler.err_out_screen('Invalid Input[i][\'Downscaling\'][\'Shortwave\'] options specified in the configuration file.')
        ...
        for optTmp in self.precipDownscaleOpt:
            if optTmp < 0 or optTmp > 1:
                err_handler.err_out_screen('Invalid Input[i][\'Downscaling\'][\'Precip\'] options specified in the configuration file.')
            if optTmp == 1:
                param_flag[count_tmp] = 1
            count_tmp = count_tmp + 1
        ...
        for optTmp in self.q2dDownscaleOpt:
            if optTmp < 0 or optTmp > 1:
                err_handler.err_out_screen('Invalid Input[i][\'Downscaling\'][\'Humidity\'] options specified in the configuration file.')
        ...
        try:
            ...
            err_handler.err_out_screen('Improper Output[\'Frequency\'] value specified  in the configuration file.')
        except KeyError:
            err_handler.err_out_screen('Unable to locate Output[\'Frequency\'] in the configuration file.')
        if self.output_freq <= 0:
            err_handler.err_out_screen('Please specify an Output[\'Frequency\'] that is greater than zero minutes.')

        # Read in the output directory
        try:
            self.output_dir = output['Dir']
        except KeyError:
            err_handler.err_out_screen('Unable to locate Output[\'Dir\'] in the configuration file.')
        ...
        try:
            ...
            err_handler.err_out_screen('Unable to locate Output[\'ScratchDir\'] in the configuration file.')
        ...
        try:
            ...
            err_handler.err_out_screen('Unable to locate Output[\'CompressOut\'] in the configuration file.')
        except ValueError:
            err_handler.err_out_screen('Improper Output[\'CompressOut\'] value.')
        ...
        try:
            ...
            err_handler.err_out_screen('Improper Output[\'FloatOutput\'] value: {}'.format(config['Output']['floatOutput']))
        ...
        try:
            self.retro_flag = int(retrospective['Flag'])
        except KeyError:
            err_handler.err_out_screen('Unable to locate Retrospective[\'Flag\'] in the configuration file.')
        except ValueError:
            err_handler.err_out_screen('Improper Retrospective[\'Flag\'] value ')
        if self.retro_flag < 0 or self.retro_flag > 1:
            err_handler.err_out_screen('Please choose a Retrospective[\'Flag\'] value of 0 or 1.')
        ...
        if self.retro_flag == 1:
            ...
            try:
                beg_date_tmp = str(retrospective['BDateProc'])
            except KeyError:
                err_handler.err_out_screen('Unable to locate Retrospective[\'BDateProc\'] in configuration file.')
                beg_date_tmp = None
            if beg_date_tmp != '-9999':
                if len(beg_date_tmp) != 12:
                    err_handler.err_out_screen('Improper Retrospective[\'BDateProc\'] length entered into the '
                                               'configuration file. Please check your entry.')
                try:
                    self.b_date_proc = datetime.datetime.strptime(beg_date_tmp, '%Y%m%d%H%M')
                except ValueError:
                    err_handler.err_out_screen('Improper Retrospective[\'BDateProc\'] value entered into the '
                                               'configuration file. Please check your entry.')
            else:
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate Retrospective[\'EDateProc\'] in configuration file.')
                end_date_tmp = None
            if end_date_tmp != '-9999':
                if len(end_date_tmp) != 12:
                    err_handler.err_out_screen('Improper Retrospective[\'EDateProc\'] length entered into the '
                                               'configuration file. Please check your entry.')
                try:
                    self.e_date_proc = datetime.datetime.strptime(end_date_tmp, '%Y%m%d%H%M')
                except ValueError:
                    err_handler.err_out_screen('Improper Retrospective[\'EDateProc\'] value entered into the '
                                               'configuration file. Please check your entry.')
        ...
        if self.retro_flag == 0:
            ...
            try:
                ...
                if self.look_back <= 0 and self.look_back != -9999:
                    err_handler.err_out_screen('Please specify a positive Forecast[\'LookBack\'] or -9999 for realtime.')
            except ValueError:
                err_handler.err_out_screen('Improper Forecast[\'LookBack\'] value entered into the '
                                           'configuration file. Please check your entry.')
            except KeyError:
                err_handler.err_out_screen('Unable to locate Forecast[\'LookBack\'] in the configuration '
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate Forecast[\'RefcstBDateProc\'] in configuration file.')
                beg_date_tmp = None
            if beg_date_tmp != '-9999':
                if len(beg_date_tmp) != 12:
                    err_handler.err_out_screen('Improper Forecast[\'RefcstBDateProc\'] length entered into the '
                                               'configuration file. Please check your entry.')
                try:
                    self.b_date_proc = datetime.datetime.strptime(beg_date_tmp, '%Y%m%d%H%M')
                except ValueError:
                    err_handler.err_out_screen('Improper Forecast[\'RefcstBDateProc\'] value entered into the '
                                               'configuration file. Please check your entry.')
            else:
                self.b_date_proc = -9999
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate Forecast[\'RefcstEDateProc\'] in configuration file.')
                end_date_tmp = None
            if end_date_tmp != '-9999':
                if len(end_date_tmp) != 12:
                    err_handler.err_out_screen('Improper Forecast[\'RefcstEDateProc\'] length entered into the'
                                               'configuration file. Please check your entry.')
                try:
                    self.e_date_proc = datetime.datetime.strptime(end_date_tmp, '%Y%m%d%H%M')
                except ValueError:
                    err_handler.err_out_screen('Improper Forecast[\'RefcstEDateProc\'] value entered into the '
                                               'configuration file. Please check your entry.')
            else:
                self.e_date_proc = -9999

            if self.e_date_proc != -9999 and self.e_date_proc <= self.b_date_proc:
                err_handler.err_out_screen('Please choose an ending Forecast[\'RefcstEDateProc\'] that is greater '
                                           'than Forecast[\'RefcstBDateProc\'].')
            ...
            try:
                ...
                err_handler.err_out_screen('Improper Forecast[\'Frequency\'] value entered into '
                                           'the configuration file. Please check your entry.')
            except KeyError:
                err_handler.err_out_screen('Unable to locate Forecast[\'Frequency\'] in the configuration '
                                           'file. Please verify entries exist.')
            if self.fcst_freq <= 0:
            ...
            if True: # was: self.realtime_flag:
                try:
                    self.fcst_shift = int(forecast['Shift'])
                except ValueError:
                    err_handler.err_out_screen('Improper Forecast[\'Shift\'] value entered into the '
                                               'configuration file. Please check your entry.')
                except KeyError:
                    err_handler.err_out_screen('Unable to locate Forecast[\'Shift\'] in the configuration '
                                               'file. Please verify entries exist.')
                if self.fcst_shift < 0:
                    err_handler.err_out_screen('Please specify a Forecast[\'Shift\'] in the configuration '
        ...
        try:
            ...
            err_handler.err_out_screen('Unable to locate Geospatial[\'GeogridIn\'] in the configuration file.')
        if not os.path.isfile(self.geogrid):
            err_handler.err_out_screen('Unable to locate necessary Geospatial[\'GeogridIn\'] file: ' + self.geogrid)

        # Check for the optional geospatial land metadata file.
        try:
            self.spatial_meta = geospatial['SpatialMetaIn']
        except KeyError:
            err_handler.err_out_screen('Unable to locate Geospatial[\'SpatialMetaIn\'] in the configuration file.')
        if len(self.spatial_meta) == 0:
            # No spatial metadata file found.
            self.spatial_meta = None
        else:
            if not os.path.isfile(self.spatial_meta):
                err_handler.err_out_screen('Unable to locate optional Geospatial[\'SpatialMetaIn\'] file: ' +
                                           self.spatial_meta)
        ...
        self.weightsDir = regridding.get('WeightsDir') if regridding else None
        ...
        try:
            ...
            err_handler.err_out_screen('Unable to locate SuppForcing[i][\'PcpType\'] in the configuration file.')
        if len(self.supp_precip_file_types) != self.number_supp_pcp:
            err_handler.err_out_screen('Number of  SuppForcing[i][\'PcpType\'] ({}) must match the number '
                                'of  SuppForcing[i][\'Pcp\'] inputs ({}) in the configuration file.'.format(len(self.supp_precip_file_types), self.number_supp_pcp))
        for fileType in self.supp_precip_file_types:
            if fileType not in ['GRIB1', 'GRIB2', 'NETCDF']:
                err_handler.err_out_screen('Invalid SuppForcing[i][\'PcpType\'] file type "{}" specified. '
                                   'Only GRIB1, GRIB2, and NETCDF are supported'.format(fileType))
        ...
        if self.number_supp_pcp > 0:
            ...
            for i,suppOpt in enumerate(self.supp_precip_forcings):
                if suppOpt < 0 or suppOpt > 5:
                    err_handler.err_out_screen('Please specify SuppForcing[i][\'Pcp\'] values between 1 and 5.')
                ...
                if suppOpt == 1 or suppOpt == 2:
                    ...
                    try:
                        ...
                        err_handler.err_out_screen('Unable to locate SuppForcing[i][\'RqiThreshold\'] in the configuration file.')
                    # Make sure the RQI threshold makes sense.
                    if self.rqiThresh[i] < 0.0 or self.rqiThresh[i] > 1.0:
                        err_handler.err_out_screen('Please specify an SuppForcing[i][\'RqiThreshold\'] between 0.0 and 1.0.')
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate SuppForcing[i][\'PcpParamDir\'] in the configuration file.')
            if len(self.supp_precip_dirs) != self.number_supp_pcp:
                err_handler.err_out_screen('Number of SuppForcing[i][\'PcpParamDir\'] must match the number of SuppForcing[i] in the configuration file.')
            ...
            if len(self.supp_precip_mandatory) != self.number_supp_pcp:
                err_handler.err_out_screen('Please specify SuppForcing[i][\'PcpMandatory\'] values for each corresponding '
                                           'supplemental precipitation options in the configuration file.')
            # Check to make sure enforcement options makes sense.
            for enforceOpt in self.supp_precip_mandatory:
                if enforceOpt < 0 or enforceOpt > 1:
                    err_handler.err_out_screen('Invalid SuppForcing[i][\'PcpMandatory\'] chosen in the configuration file. '
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate SuppForcing[i][\'RegridOptPcp\'] in the configuration file.')
            if len(self.regrid_opt_supp_pcp) != self.number_supp_pcp:
                err_handler.err_out_screen('Please specify SuppForcing[i][\'RegridOptPcp\'] values for each corresponding supplemental '
                                           'precipitation product in the configuration file.')
            # Check to make sure regridding options makes sense.
            for regridOpt in self.regrid_opt_supp_pcp:
                if regridOpt < 1 or regridOpt > 3:
                    err_handler.err_out_screen('Invalid SuppForcing[i][\'RegridOptPcp\'] chosen in the configuration file. '
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate SuppForcing[i][\'PcpTemporalInterp\'] in the configuration file.')
            if len(self.suppTemporalInterp) != self.number_supp_pcp:
                err_handler.err_out_screen('Please specify SuppForcing[i][\'PcpTemporalInterp\'] values for each '
            ...
            for temporalInterpOpt in self.suppTemporalInterp:
                if temporalInterpOpt < 0 or temporalInterpOpt > 2:
                    err_handler.err_out_screen('Invalid SuppForcing[i][\'PcpTemporalInterp\'] chosen in the configuration file. '
                                               'Please choose a value of 0-2 for each corresponding input forcing')
            ...
            try:
                ...
                err_handler.err_out_screen('Unable to locate SuppForcing[i][\'PcpInputOffsets\'] in the configuration file.')
            if len(self.supp_input_offsets) != self.number_supp_pcp:
                err_handler.err_out_screen('Please specify SuppForcing[i][\'PcpInputOffsets\'] values for each '
                                           'corresponding input forcings for SuppForcing.')
            # Check to make sure the input offset options make sense. There will be additional
            # checking later when input choices are mapped to input products.
            for inputOffset in self.supp_input_offsets:
                if inputOffset < 0:
                    err_handler.err_out_screen(
                            'Please specify SuppForcing[i][\'PcpInputOffsets\'] values greater than or equal to zero.')
        ...
        for optTmp in self.input_forcings:
            if optTmp == 7:
                ...
                if self.cfsv2EnsMember < 1 or self.cfsv2EnsMember > 4:
                    err_handler.err_out_screen('Please chose an Ensembles[\'cfsEnsNumber\'] value of 1,2,3 or 4.')
